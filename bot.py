...
from olcrtc import gen_key, gen_room, systemctl, journalctl
...
async def new_config_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Generate client_id from name (lowercase, no special chars)
    client_id = re.sub(r"[^a-zA-Z0-9]", "", name).lower() or name.lower()
    context.user_data["name"] = name
    context.user_data["client_id"] = client_id

    msg = await update.message.reply_text("⏳ Генерирую ключ...")

    try:
        key_hex = await gen_key()
    except Exception as e:
        await msg.edit_text(f"Ошибка генерации ключа: {e}")
        return ConversationHandler.END

    context.user_data["key_hex"] = key_hex

    await msg.edit_text("⏳ Создаю Jazz-комнату...")

    try:
        room_result = await gen_room(client_id, key_hex)
    except Exception as e:
        await msg.edit_text(f"Ошибка создания комнаты: {e}")
        return ConversationHandler.END

    room_id = room_result.get("room_id")
    if not room_id:
        raw = room_result.get("raw", "нет")[:300]
        await msg.edit_text(f"Не удалось создать комнату.\nВывод: {raw}")
        return ConversationHandler.END

    await create_profile(name, client_id, key_hex, room_id)
    ...
    # Save room_id for message delivery
    context.user_data["_last_room"] = room_id
    context.user_data["_last_key"] = key_hex
    context.user_data["_last_cid"] = client_id
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        fallbacks=[
            CallbackQueryHandler(new_config_cancel, pattern="^menu$"),
        ],
    )
    ...
    app.add_handler(CallbackQueryHandler(do_start, pattern="^do_start:\\d+$"))

    # ⏹ Остановить
    app.add_handler(CallbackQueryHandler(stop_service, pattern=f"^{CB_STOP}$"))
    app.add_handler(CallbackQueryHandler(do_stop, pattern="^do_stop:\\d+$"))
